...
const ProductPage = () => {
  ...
  const {
    products: { product, isLoading },
    comments,
  } = useSelector((state) => state);
  const [comment, setComment] = useState('');
  const currentComments = comments.comments.filter(
  ...
  const onAddComment = () => {
    setComment('');
    dispatch(
      addComment({ description: comment, productId: id, date: datenow })
    );
  };
